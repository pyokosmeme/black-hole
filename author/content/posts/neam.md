...
<p>
First, we formalize the systemic force <em>F</em><sub>sys</sub> by transitioning from intensive network coordinates to extensive market volume. We define the extensive <strong>Aggregate Excess Demand Vector</strong> (D<sub>t</sub>) as the total sum of all individual agent action vectors, scaling directly with population size <em>N</em> and the mean-field magnetization:
</p>

<div class="eq-block">
D<sub>t</sub> = ∑<sub>i=1</sub><sup>N</sup> <em>x<sub>i</sub></em><sup>(t)</sup> = <em>N M<sub>t</sub></em>
</div>

<p>
Because market impact is driven by this total extensive order flow crossed with Kyle's depth parameter <em>λ</em>, the deterministic systemic force driving price drift is governed precisely by the scalar projection <em>λ (w<sup>T</sup> D<sub>t</sub>)</em>. 
</p>
...
<div class="eq-block">
d<em>S<sub>t</sub></em> = [ <em>λ</em> (<em>w<sup>T</sup> D<sub>t</sub></em>) ] <em>S<sub>t</sub></em> dt + [ <em>σ<sub>v</sub> N √HHI<sub>t</sub></em> ] <em>S<sub>t</sub></em> d<em>W<sub>t</sub></em>
</div>
