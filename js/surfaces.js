...
(function () {
  ...
  function setupThree(mount) {
    ...
    // Terrain displacement runs in the GPU vertex shader using Ashima 3D simplex
    // noise (the canonical GLSL simplex — same character as the original JS
    // SimplexNoise). No per-frame CPU vertex loop, no buffer re-upload; the
    // mesh is static. The plane is oversized so the camera frustum never sees
    // past an edge into the background.
    const gridRes = 300;
    const geometry = new THREE.PlaneGeometry(500, 500, gridRes, gridRes);
    const uniforms = {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        // Ashima Arts 3D simplex noise — MIT license. Matches the JS
        // SimplexNoise.noise3D the original CPU version used.
        ...
        void main(){
          ...
          float n = snoise(vec3(p.x*0.012, (p.y+scroll)*0.012, morph));
          float h = n * 8.5;                        // ±8.5 amplitude, centered on 0
