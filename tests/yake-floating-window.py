...
with sync_playwright() as p:
    ...
    for width,height in [(1366,768),(1920,1080),(390,844),(320,568),(568,320),(844,390)]:
        ...
        for world in ['jin','shu','marassa','chawkee','skarda','plomo','gullinkambi','celosia','five','mun','in','sin','island-mu','yong','pani']:
            ...
            assert page.locator('.card-close').evaluate('''e=>{const r=e.getBoundingClientRect(),c=e.closest('#scene-card'),s=getComputedStyle(c),b=c.getBoundingClientRect(),h=c.querySelector('.card-heading').getBoundingClientRect();return Math.abs(r.top-h.top)<1&&Math.abs(b.right-r.right-parseFloat(s.borderRightWidth)-parseFloat(s.paddingRight))<1&&r.width>=44&&r.height>=44&&e.contains(document.elementFromPoint(r.x+r.width/2,r.y+r.height/2))}'''),('close alignment',world,width,height)
